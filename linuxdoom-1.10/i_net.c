...
void PacketGet(void) {
    ...
    {
        static int first = 1;
        if(first) printf("len=%d:p=[0x%x 0x%x] \n", c, *(int*)&sw, *((int*)&sw + 1));
        first = 0;
    }
...
int GetLocalAddress(void) {
    ...
    if(!hostentry) I_Error("GetLocalAddress : gethostbyname: couldn't get local host");
...
void I_InitNetwork(void) {
    ...
    while(++i < myargc && myargv[i][0] != '-') {
        ...
        if(myargv[i][0] == '.') {
            ...
            if(!hostentry) I_Error("gethostbyname: couldn't find %s", myargv[i]);
