...
void I_ShutdownGraphics(void) {
    ...
    if(!XShmDetach(X_display, &X_shminfo)) I_Error("XShmDetach() failed in I_ShutdownGraphics()");
            ...
            if(!rc) {
                if(shminfo.shm_nattch) {
                    ...
                    if(getuid() == shminfo.shm_perm.cuid) {
                        ...
                        if(id == -1) I_Error("Could not get shared memory");
...
void I_InitGraphics(void) {
    ...
    if(doShm) {
        ...
        if(!XShmAttach(X_display, &X_shminfo)) I_Error("XShmAttach() failed in InitGraphics()");
